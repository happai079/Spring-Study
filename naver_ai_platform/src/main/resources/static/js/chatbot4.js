					...
					if(bubbles[b].type == 'text'){ // 기본 답변인 경우
						...
						if(bubbles[b].data.cover.type=="image"){//이미지 이면
							...
							if(bubbles[b].data.contentTable == null){
								...
								$("#chatBox").append('<div class="msgBox receive"><p>' + bubbles[b].data.cover.data.description+ "</p></div><br>");	
								// 챗봇으로 부터 받은 텍스트 답변을 음성으로 변환하기 위해 TTS 호출									   
						...
						for(var ct in bubbles[b].data.contentTable){
							...
							for(var ct_d in ct_data){
								$("#chatBox").append
								("<div class='msgBox receive'>"+"<a href='"+ct_data[ct_d].data.data.action.data.url+"' target='_blank'>" + 
									ct_data[ct_d].data.data.action.data.url+ "</a></div><br>");
						    }
					    }// contentTable for문 끝
				    }//template 끝			
				}//bubbles for문 종료
