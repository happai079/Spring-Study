	...
	public String imgLinkMainService(String voiceMessage) {
        ...
        try {
            ...
            if(responseCode==200) { // Normal call
                ...
                // 서비스에서 JSON 파싱하지 않고 cahtbot4.js로 그대로 전송
                // chatbot4.js에서 자바스크립트로 JSON 파싱
                // chatbotMessage = jsonToString(chatbotMessage);
...
//=> userId is a unique code for each chat user, not a fixed value, recommend use UUID. use different id for each user could help you to split chat history for users.
    ...
    public String jsonToString(String jsonResultStr) {
    	...
    	try {
			...
			JSONArray bubbleArray = (JSONArray) jsonObj.get("bubbles");
			JSONObject obj0 = (JSONObject) bubbleArray.get(0);
			
			JSONObject dataObj = (JSONObject) obj0.get("data");
			
			result = dataObj.getString("description");
		} catch (Exception e) {
			e.printStackTrace();
		}
    	return result;
