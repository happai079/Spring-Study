...
public class CFRFaceRecogService {
	...
	public ArrayList<FaceVO> faceRecog(String filePathName) {
        ...
        try {
            ...
            if(br != null) {
                ...
                br.close();
                System.out.println(response.toString()); // 서버로부터 받은 JSON 결과 출력
                faceList = jsonToList(response.toString());
            } else {
	...
	// API 서버로부터 받은 JSON 형태의 데이터에서 value오 confidence 추출해서
	// VO리스트 만들어 반환
	public ArrayList<FaceVO> jsonToList(String jsonResultStr){
		ArrayList<FaceVO> faceList = new ArrayList<FaceVO>();
		...
		try {
			...
			// JSON 형태의 묹열에서 JSON 오브젝트 "faces" 추출해서 JSONArray에 저장
			if(faceArray != null) {
				// JSONArray의 각 요소에서 value, confidence 추출
